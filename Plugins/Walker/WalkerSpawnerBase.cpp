...
void AWalkerSpawnerBase::BeginPlay()
{
  ...
  if (bSpawnWalkers && bSpawnWalkersAtBeginPlay) {
    ...
    if ( NumberOfWalkers > 1 ) {
      ...
      PermanentStart=BeginSpawnPoints[139];
      TryToSpawnWalkerAt(*PermanentStart);
      ++Count;
    }
...
bool AWalkerSpawnerBase::TryGetValidDestination(const FVector &Origin, FVector &Destination)
{
  ...
  if ( NumberOfWalkers > 1 ) {
    ...
    Destination = SpawnPoints[1]->GetActorLocation();
    return true;
  }
...
bool AWalkerSpawnerBase::TryToSpawnWalkerAt(const AWalkerSpawnPointBase &SpawnPoint)
{
  ...
  trans=SpawnPoint.GetActorTransform();
  pos=trans.GetLocation();
  // Add walker and set destination.
  Walkers.Add(Walker);
  printf("Walker is born at (%f,%f,%f)!\n",pos.X,pos.Y,pos.Z);
  printf("Walker is ended at (%f,%f,%f)!\n",Destination.X,Destination.Y,Destination.Z);
