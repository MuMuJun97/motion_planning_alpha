...
void AWheeledVehicleAIController::Tick(const float DeltaTime)
{
  ...
  if(m_bAutopilot)
  // printf("the car's location is (%f,%f,%f)\n",CurrentLocation.X,CurrentLocation.Y,CurrentLocation.Z);
    ...
    const bool bOverwriteCurrent)
{
  ...
  printf("Now TargetLocations have %lu nodes\n",TargetLocations.size());
}
...
void AWheeledVehicleAIController::TickAutopilotController()
{
  ...
  if (!TargetLocations.empty()) {
    if(m_bAutopilot)
      printf("TargetLocations have %lu nodes\n",TargetLocations.size());
    Steering = GoToNextTargetLocation(Direction);
  } else {
    //if(m_bAutopilot)
    printf("TargetLocations is empty, Stop\n");
    AutopilotControl.Throttle = Stop(Vehicle->GetVehicleForwardSpeed() * 0.036f);
...
float AWheeledVehicleAIController::GoToNextTargetLocation(FVector &Direction)
{
  ...
  if (Target.Equals(CurrentLocation, 80.0f)) {
    ...
    if (!TargetLocations.empty()) {
      ...
      return 0.0f;// return CalcStreeringValue(Direction);
    }
  }
