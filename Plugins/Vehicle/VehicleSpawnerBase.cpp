...
void AVehicleSpawnerBase::BeginPlay()
{
  ...
  NumberOfVehicles = 1;//FMath::Max(0, NumberOfVehicles);
...
;
  ...
  if (bSpawnVehicles) 
  {
	...
	{
	  SpawnVehicleAtSpawnPoint(*SpawnPoints[16]);
    ...
    const APlayerStart &SpawnPoint)
{
  ...
  if ((Vehicle != nullptr) && !Vehicle->IsPendingKill())
  {
	...
	{ // Sometimes fails...
      ...
      //Controller->MoveToLocation(SpawnPoints[17]->GetActorLocation(),-1.0f,false,true,true,true,nullptr,true)
      // paths.Push(SpawnPoint.GetActorLocation());
      paths.Push(SpawnPoints[147]->GetActorLocation());
      paths.Push(SpawnPoints[123]->GetActorLocation());
      paths.Push(SpawnPoints[144]->GetActorLocation());
      Controller->SetFixedRoute(paths);
      Controller->SetSpeedLimit( 5.0f );
