...
int ou_route_respond::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int ou_route_respond::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;
...
int ou_route_respond::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t ou_route_respond::getHash()
{
    static int64_t hash = _computeHash(NULL);
    return hash;
}

const char* ou_route_respond::getTypeName()
{
    return "ou_route_respond";
}

int ou_route_respond::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int ou_route_respond::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;
...
int ou_route_respond::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
...
int64_t ou_route_respond::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == ou_route_respond::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, (void*)ou_route_respond::getHash };
